...
test_that("lapply_species works", {
  ...
  results <- lapply_species(validate_species)
...
test_that("lapply_kfold_species works", {
  ...
  results <- lapply_kfold_species(validate_species, species_check=species_list[3:15,1], folds_check=3:4, k=3:4, species=species_list[3:15,], fold_type="random")
  expect_equal(lenght(results), 5)
